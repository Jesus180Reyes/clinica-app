...
export const useHistorialMedico = () => {
  ...
  const url = '/api/historial-medico';
    ...
    const createHistorial = async(data: any):Promise<boolean> => {
      if(data.diagnostico.length === 0 || data.tratamiento.length === 0 ) return false;
        try {
        setStatus(Status.inProgress);
        ...
        console.log(error);
        CustomModals.showCustomModal('Upps a ocurrido un error no esperado, Vuelve a intentarlo', 'error',error.message );
