 ...
 */
export const CustomTextfieldComponent:FC<Props> = ({title, value,onChange, error,disabled,placeholder,typeInput  = 'text'}) => {
  return (
    <>
     <label className='font-semibold text-sm text-gray-600 pb-1 block'>{title}</label>
    <input type={typeInput} disabled={disabled} placeholder={placeholder} onChange={onChange} value={value} className={`border rounded-lg px-3 py-2 mt-1 mb-3 text-sm w-full ${error ? 'border-red-500' : undefined}`} />
